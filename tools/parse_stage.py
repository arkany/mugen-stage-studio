...
class DefInfo:
    ...
    zoomin: float | None = None
    zoomout: float | None = None
    startzoom: float | None = None
...
def parse_def(path: Path) -> DefInfo:
    ...
    if camera:
        ...
        # MUGEN 1.1 / IKEMEN GO stage zoom. `zoomout` is a divisor on the
        # viewport: at 0.75 the camera shows 1/0.75 of the normal area, so
        # every bound has to shrink to match. Ignoring it is why zoom-enabled
        # stages appear to "scroll past the background edge".
        info.zoomin = _to_float(camera.get("zoomin"))
        info.zoomout = _to_float(camera.get("zoomout"))
        info.startzoom = _to_float(camera.get("startzoom"))
...
def analyze_pair(sff_path: Path, def_path: Path, label: str | None = None) -> dict[str, Any]:
    ...
    if result.get("def"):
        ...
        if result.get("sprite"):
            ...
            zoomout = d.get("zoomout") or 1.0
            if zoomout <= 0:
                zoomout = 1.0

            # Zooming out enlarges the viewport, shrinking every bound.
            visible_w = lcw / zoomout
            visible_h = lch / zoomout

            # Resolve where the artwork actually sits. `[BG] start` places the
            # sprite's AXIS point, measured from the top of the viewport - it
            # is not relative to the floor line. Assuming a flush-bottom mount
            # (the old behaviour) mis-derives boundhigh for every stage that
            # uses a top-anchored or offset axis, which is most of them.
            placement = _resolve_placement(sprite, d)
            derived["placement"] = placement

            if placement is None:
                derived["expectedBoundleft"] = None
                derived["expectedBoundright"] = None
                derived["expectedBoundhigh"] = None
                derived["boundsMatchDerived"] = None
                derived["boundsVerdict"] = "no-placement"
            else:
                # Horizontal: a layer with delta d moves d units per camera
                # unit, so it tolerates camera travel of slack/(2*delta). The
                # binding constraint is the layer that runs out first.
                deltas = _layer_deltas(d) or [1.0]
                horiz = min(
                    max(0.0, (sprite["width"] - visible_w) / (2.0 * dx))
                    for dx in deltas
                    if dx > 0
                )
                ebr = int(horiz)
                ...
                extra_h = max(0.0, visible_h - lch)
                ebh = -int(max(0.0, -placement["top"] - extra_h / 2.0))

                derived["expectedBoundleft"] = ebl
                derived["expectedBoundright"] = ebr
                derived["expectedBoundhigh"] = ebh

                # A stage is correct when its shipped bounds stay INSIDE the
                # derived ceiling - holding margin back is good practice, not
                # an error. Exceeding it means the camera can show past the
                # edge of the artwork.
                abl, abr = d.get("boundleft"), d.get("boundright")
                abh = d.get("boundhigh")
                within = []
                if abl is not None:
                    within.append(abl >= ebl)
                if abr is not None:
                    within.append(abr <= ebr)
                if abh is not None:
                    within.append(abh >= ebh)
                derived["boundsMatchDerived"] = bool(within) and all(within)
                derived["boundsVerdict"] = (
                    "within-artwork" if derived["boundsMatchDerived"] else "exceeds-artwork"
                )
                derived["boundhighWithinArtwork"] = (
                    None if abh is None else abh >= ebh
                )
...
def _parse_pair(value, default=None):
    """Parse a `x, y` DEF value into a float 2-tuple."""
    if not value:
        return default
...
def _resolve_placement(sprite, d):
    """
    Where the artwork's edges land in screen space.

    Screen space has y = 0 at the TOP of the viewport. A `[BG] start = sx, sy`
    puts the sprite's axis at (sx, sy), so the artwork's top edge is at
    `sy - axisY`. Axis and start are a coupled pair; neither means anything
    without the other.
    """
    start = _parse_pair(d.get("bg_start"))
    if start is None:
        return None
    ax, ay = sprite.get("axisX"), sprite.get("axisY")
...
def _layer_deltas(d):
    """Horizontal delta of every BG layer that declares one."""
    out = []
    for el in d.get("bg_elements") or []:
        pair = _parse_pair(el.get("delta"))
